...
from .clip_model import Clip_text
...
__all__ = ['MiT_B0', 'MiT_B1', 'MiT_B2', 'get_segformer', 'get_segformer_multi']
...
class Segformer_multi(nn.Module):
    def __init__(
        self, 
        pretrained=None,
        img_size=1024, 
        ...
        # self.linear_fuse1 = nn.Sequential(*[nn.Conv2d(4 * decoder_dim, decoder_dim, 1),
        #                                    batchnorm_layer(decoder_dim),
        #                                    nn.ReLU()])
        self.dropout1 = nn.Dropout2d(drop_rate)
        self.linear_pred1 = nn.Conv2d(decoder_dim, 1, kernel_size=1)
        if self.task_len == 3:
            self.dropout2 = nn.Dropout2d(drop_rate)
            self.linear_pred2 = nn.Conv2d(decoder_dim, 3, kernel_size=1)            
    ...
    def zero_grad_shared_modules(self):
        for p in [p for name,p in self.named_parameters() if p.requires_grad and not any(excluded in name for excluded in ['linear_pred'])]:
            p.grad=None
    ...
    def ScaledDotProductAttention(self, q,k,v):
        ...
        N,C,H,W = v.shape
        temperature = (C*H*W) ** 0.5
        # print(k.transpose(0,1).view(C,-1).shape)
        attn =  q/temperature @ k.transpose(0,1).view(C,-1) #q: [1,C], K: [N,C,H,W]; [1,C] @ [C,NHW]
        attn = attn.view(N,1,H,W)

        attn = self.dropout_att(F.softmax(attn, dim=0)) #[N,1,H,W], N=8, thus attn[0]~=0.15
        output = torch.mul(attn, v) # [N,1,H,W] * [N,C,H,W] = [N,C,H,W]
        output = self.dropout_out(self.clip_map(output.transpose(1,3).contiguous()))
        output = output.transpose(1,3).contiguous() + v
        # output = self.clip_map(torch.cat([output, v], dim=1))
    ...
    def forward(self, x):
        ...
        # _c_1 = self.linear_fuse1(torch.cat([_c4, _c3, _c2, _c1], dim = 1))
        # x1 = self.dropout1(_c_1)
        x1 = self.dropout1(_c)
        x1 = self.linear_pred1(x1)
        if self.task_len == 3:
            x2 = self.dropout2(_c)
...
def MiT_B0(pretrained, img_size, num_classes, batchnorm_layer):
    ...
    return model
def MiT_B0_multi(pretrained, img_size, num_classes, task_len, batchnorm_layer):
    model = Segformer_multi(
        pretrained=pretrained,
        img_size=img_size,
        patch_size=4, 
        num_classes=num_classes,
...
def MiT_B1(pretrained, img_size, num_classes, batchnorm_layer):
    ...
    return model
def MiT_B1_multi(pretrained, img_size, num_classes, batchnorm_layer):
    model = Segformer_multi(
        pretrained=pretrained,
        img_size=img_size,
        patch_size=4, 
...
def get_segformer(backbone='MiT_B0', pretrained=None, img_size=[512,1024], num_class=40, norm_layer=nn.BatchNorm2d, **kwargs):
    if backbone == 'MiT_B0':
        return MiT_B0(pretrained, img_size, num_class, norm_layer)
    elif backbone == 'MiT_B1':
        return MiT_B1(pretrained, img_size, num_class, norm_layer)
    elif backbone == 'MiT_B2':
        return MiT_B2(pretrained, img_size, num_class, norm_layer)
    elif backbone == 'MiT_B3':
        return MiT_B3(pretrained, img_size, num_class, norm_layer)
    elif backbone == 'MiT_B4':
        return MiT_B4(pretrained, img_size, num_class, norm_layer)
    else:
        raise ValueError('no such backbone')

def get_segformer_multi(backbone='MiT_B0', pretrained=None, img_size=[512,1024], num_class=40, task_len=None, norm_layer=nn.BatchNorm2d, **kwargs):
    if backbone == 'MiT_B0':
        return MiT_B0_multi(pretrained, img_size, num_class, task_len, norm_layer)
    elif backbone == 'MiT_B1':
        return MiT_B1_multi(pretrained, img_size, num_class, norm_layer)
    elif backbone == 'MiT_B2':
        return MiT_B2(pretrained, img_size, num_class, norm_layer)
    elif backbone == 'MiT_B3':
        return MiT_B3(pretrained, img_size, num_class, norm_layer)
